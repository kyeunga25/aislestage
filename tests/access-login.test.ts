...
describe('Access login routing', () => {
  ...
  it('builds a public login URL without accepting arbitrary reason values', () => {
    ...
    expect(normalizeAccessFailureReason('authentication-expired')).toBe('authentication-expired')
    expect(normalizeAccessFailureReason('authentication-audience-mismatch')).toBe('authentication-audience-mismatch')
    expect(normalizeAccessFailureReason('authentication-issuer-mismatch')).toBe('authentication-issuer-mismatch')
    expect(normalizeAccessFailureReason('authentication-signature-invalid')).toBe('authentication-signature-invalid')
