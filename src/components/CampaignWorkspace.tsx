import { Check, FileImage, ImagePlus, LoaderCircle, Plus, ShieldCheck, UploadCloud, X } from 'lucide-react'
...
export function CampaignWorkspace(props: Props) {
  const { brand, product, intent, image, agentState, agentBusy, generationAvailable, onBrandChange, onProductChange, onIntentChange, onImageSelected, onPlan, onApprove, onGenerate, onRevise } = props
  const inputRef = useRef<HTMLInputElement>(null)
  const factsReady = Boolean(brand.name && product.name && product.price && product.promotion && product.benefits.filter(Boolean).length >= 2)
  ...
  return <>
    ...
    <div className="studio-grid">
      ...
      <section className="product-panel" aria-labelledby="product-image-title">
        ...
        <div className="product-canvas"><img src={image.url} alt={`${product.name} 商品原圖`} /></div>
        <button className="upload-zone" type="button" onClick={() => inputRef.current?.click()} disabled={image.status === 'uploading'}>
          ...
          <span><strong>{image.status === 'uploading' ? '正在加密上傳…' : '更換商品圖片'}</strong><small>支援 JPG、PNG、WebP，最大 8 MB</small></span>
        </button>
        <input ref={inputRef} className="visually-hidden" type="file" accept="image/png,image/jpeg,image/webp" onChange={chooseImage} />
        <div className={`asset-row ${image.status}`}><FileImage size={17} /><span><strong>{image.name}</strong><small>{image.status === 'ready' ? '已儲存在此工作區的私人素材庫' : image.status === 'error' ? image.error : image.status === 'uploading' ? '正在處理檔案' : '本機示範素材'}</small></span><button type="button" onClick={() => inputRef.current?.click()} aria-label="更換圖片"><ImagePlus size={16} /></button></div>
      </section>

      <CampaignAgentPanel state={agentState} busy={agentBusy} generationAvailable={generationAvailable} onPlan={onPlan} onApprove={onApprove} onGenerate={onGenerate} onRevise={onRevise} />
    </div>
  </>
}
