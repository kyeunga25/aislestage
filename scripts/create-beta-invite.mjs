...
const email = (process.env.AISLEPACK_INVITE_EMAIL || flag('--email') || '').trim().toLowerCase()
const database = (process.env.AISLEPACK_INVITE_DATABASE || flag('--database') || '').trim()
const config = (process.env.AISLEPACK_WRANGLER_CONFIG || flag('--config') || 'wrangler.local.jsonc').trim()
const days = Number(flag('--days') || '7')
const accountType = flag('--account-type') === 'test' ? 'test' : 'beta'
const location = process.argv.includes('--local') ? '--local' : '--remote'

if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new Error('Set AISLEPACK_INVITE_EMAIL to a valid recipient email.')
if (!/^[a-z0-9][a-z0-9._-]{1,127}$/i.test(database)) throw new Error('Set AISLEPACK_INVITE_DATABASE to the protected D1 database name.')
