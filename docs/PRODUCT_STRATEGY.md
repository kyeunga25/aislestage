...
This document replaces application-driven planning with a product-evidence plan. The product-facing working name is **AislePack**: “aisle” anchors the product in retail while “pack” names the fixed multi-format outcome. Use the descriptor **AI 電商素材包** wherever the brand first appears. Infrastructure identifiers keep the `motive` codename until naming checks are complete.
...
Current product-facing hierarchy:

- brand: `AislePack`;
- plain-language descriptor: `AI 電商素材包`;
- core outcome: `Campaign Pack`;
- primary promise: `一張商品圖，完成整套推廣素材`.

Avoid using `CampaignKit` or `PromoPack`: both already have active products in adjacent marketing workflows. `AislePack` remains a working brand until a formal Hong Kong company-name, trademark, domain, and customer-language review is completed.
